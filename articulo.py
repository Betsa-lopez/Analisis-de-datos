...
def mapear_likert(valor):
    ...
    elif v.startswith('2'): return 2
    elif v.startswith('3'): return 3
    elif v.startswith('4'): return 4
    elif v.startswith('5'): return 5
    return np.nan
...
def cargar_datos(archivo):
    ...
    try:
        ...
        print(f"  Asegúrate de que el archivo Excel esté en la misma carpeta que este script.")
        sys.exit(1)
...
def estadisticos_descriptivos(df, cols_b):
    ...
    for col, etiq in zip(cols_b, etiquetas):
        ...
        medias.append(media)
        sigmas.append(sigma)
        marca = " ◄ más alto" if media == max([df[c].mean() for c in cols_b]) else ""
        print(f"  {etiq:<42} {media:>8.4f} {sigma:>8.4f} {minv:>6.0f} {maxv:>6.0f}{marca}")
    ...
    print(f"  {'Diferencia respecto al punto neutral:':<42} {prom_general - MU:>8.4f}")
...
def prueba_z(datos, nombre_grupo, mu=MU):
    ...
    print(f"  Denominador    = σ/√n   = {sigma:.4f}/{raiz_n:.4f} = {denominador:.4f}")
    print(f"  Z              = {numerador:.4f} / {denominador:.4f} = {z:.4f}")
    print(f"  |Z|            = {abs(z):.4f}")
    print(f"  Valor crítico  = {Z_CRITICO}")
    if rechaza:
        print(f"  Decisión       ✔ Se RECHAZA H₀  (|Z| = {abs(z):.4f} > {Z_CRITICO})")
    else:
        print(f"  Decisión       ✘ No se rechaza H₀  (|Z| = {abs(z):.4f} ≤ {Z_CRITICO})")
...
def grafica_semestres(df, col_semestre):
    ...
    bars = ax.bar(etiq, cont, color=cols, edgecolor='white', linewidth=0.8, zorder=3)
    ...
    ax.set_title('Figura 1. Distribución de participantes por semestre (n = 235)', fontsize=11, pad=12)
    ax.set_ylim(0, 58)
    ax.grid(axis='y', linestyle='--', alpha=0.4, zorder=0)
    ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
    leyenda = [mpatches.Patch(color=AZUL_CLAR, label='Semestres iniciales (1°–3°)'),
               mpatches.Patch(color=AZUL_MED,  label='Semestres intermedios (4°–6°)'),
...
def grafica_sexo(df, col_sexo):
    ...
    sizes   = [conteos.get("Hombre",0), conteos.get("Mujer",0), conteos.get("Prefiero no decirlo",0)]
    ...
    for tx in texts: tx.set_fontsize(9)
    ax.set_title('Figura 2. Distribución por sexo (n = 235)', fontsize=11, pad=12)
    plt.tight_layout()
...
def grafica_edad(df, col_edad):
    ...
    ax.set_title('Figura 3. Distribución por rango de edad (n = 235)', fontsize=11, pad=12)
    ax.set_xlim(0, 160)
...
def grafica_horas(df, col_horas):
    orden_h = ['Menos de 2 horas','2-4 horas','5-7 horas','8-10 horas','11-12 horas','Más de 12 horas']
    ...
    bars = ax.bar(etiq_h, cont_h, color=cols_h, edgecolor='white', linewidth=0.8, zorder=3)
    ...
    ax.set_ylabel('Número de estudiantes', fontsize=10)
    ax.set_title('Figura 4. Horas continuas de programación por sesión (n = 235)', fontsize=11, pad=12)
    ax.set_ylim(0, 100)
    ax.grid(axis='y', linestyle='--', alpha=0.4, zorder=0)
    ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
    leyenda_h = [mpatches.Patch(color=AZUL_CLAR, label='Jornada regular (<4 h)'),
                 mpatches.Patch(color=AZUL_MED,  label='Jornada moderada (5–7 h)'),
...
def grafica_extremas(df, col_extremas):
    orden_x = ['No, nunca','Sí, una vez','Sí, 2-3 veces','Sí, 4 o más veces']
    ...
    ax.set_ylabel('Número de estudiantes', fontsize=10)
    ax.set_title('Figura 5. Frecuencia de jornadas extremas\n(3 o más días consecutivos, n = 235)', fontsize=11, pad=12)
    ax.set_ylim(0, 105)
    ax.grid(axis='y', linestyle='--', alpha=0.4, zorder=0)
    ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
    plt.tight_layout()
...
def grafica_reactivos(medias, sigmas):
    ...
    ax.set_title('Figura 6. Media por reactivo — Impacto cognitivo (n = 235)', fontsize=11, pad=12)
    ax.set_ylim(1, 5)
...
def grafica_prueba_z(res1, res2, titulo_h, etiq1, etiq2):
    ...
    for ax, res, etiq, color in zip(axes, [res1, res2], [etiq1, etiq2], [AZUL_CLAR, AZUL]):
        ...
        ax.fill_between(x, y, where=(x < -Z_CRITICO), color='#FF6B6B', alpha=0.5, label='Zona rechazo')
        ax.fill_between(x, y, where=(x > Z_CRITICO),  color='#FF6B6B', alpha=0.5)
        ax.fill_between(x, y, where=(x >= -Z_CRITICO) & (x <= Z_CRITICO), color='#A8C4E0', alpha=0.3, label='Zona no rechazo')
        ax.axvline(x=res['z'], color=color, linewidth=2.5, linestyle='-', label=f"Z = {res['z']}")
        ax.axvline(x=-Z_CRITICO, color='red', linewidth=1, linestyle='--', label=f'±{Z_CRITICO}')
    ...
    plt.tight_layout()
    nombre = titulo_h.replace(' ', '_').replace('—','').replace('/','vs')[:30] + '.png'
    guardar(fig, nombre)
...
def main():
    ...
    # 2. Procesar
    df, col_semestre, col_sexo, col_edad, col_horas, col_extremas, cols_b = procesar_datos(df)
    ...
    print(f"  Fórmula: z = (X̄ − μ) / (σ / √n)   |   μ = {MU}   |   α = {ALPHA}")
    print(f"{'='*60}")

    regular = df[df[col_horas].isin(['Menos de 2 horas','2-4 horas'])]['promedio_cognitivo'].dropna()
    extensa  = df[df[col_horas].isin(['8-10 horas','11-12 horas'])]['promedio_cognitivo'].dropna()
    ...
    iniciales = df[df[col_semestre].isin(['Primer semestre','Segundo semestre','Tercer semestre'])]['promedio_cognitivo'].dropna()
    avanzados = df[df[col_semestre].isin(['Séptimo semestre','Octavo semestre','Noveno semestre o superior'])]['promedio_cognitivo'].dropna()
    ...
    print(f"{'='*60}")
    print(f"\n  Hipótesis 1:")
    print(f"    Jornada regular  → Z = {res_h1_g1['z']:>6.4f}  {'✔ Se rechaza H₀' if res_h1_g1['rechaza'] else '✘ No se rechaza H₀'}")
    print(f"    Jornada extensa  → Z = {res_h1_g2['z']:>6.4f}  {'✔ Se rechaza H₀' if res_h1_g2['rechaza'] else '✘ No se rechaza H₀'}")
    print(f"\n  Hipótesis 2:")
    print(f"    Sem. iniciales   → Z = {res_h2_g1['z']:>6.4f}  {'✔ Se rechaza H₀' if res_h2_g1['rechaza'] else '✘ No se rechaza H₀'}")
    print(f"    Sem. avanzados   → Z = {res_h2_g2['z']:>6.4f}  {'✔ Se rechaza H₀' if res_h2_g2['rechaza'] else '✘ No se rechaza H₀'}")
